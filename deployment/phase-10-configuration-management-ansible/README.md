...
This phase teaches Ansible in two stages, the same structure as Phase 7 (CI/CD) and Phase 9 (Terraform):

```text
deployment/phase-10-configuration-management-ansible/
+-- phase-10-ansible-basics/          (start here)
+-- phase-10-ansible-production/      (do this second)
```
...
Do `phase-10-ansible-basics` first — the production lab assumes you know inventories, modules, playbooks, variables, templates, and handlers.

## Terraform vs Ansible: Why Both Exist

Phase 9 (Terraform) **creates machines**. This phase (Ansible) **configures what runs inside machines that already exist**.
...
Real companies commonly run both: `terraform apply` builds the servers, then `ansible-playbook` configures them. (Terraform's user data — which you used in Phase 9 — covers simple first-boot setup; Ansible takes over when configuration is bigger than one script, changes over time, or spans many hosts.)
...
After both labs, move to Phase 11 (Observability) — with provisioning (Terraform) and configuration (Ansible) automated, the next skill is seeing what your platform is actually doing: metrics, dashboards, logs, and traces.
